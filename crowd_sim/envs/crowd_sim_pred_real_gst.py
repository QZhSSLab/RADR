...
class CrowdSimPredRealGST(CrowdSimPred):
    ...
    def generate_ob(self, reset, sort=False):
        ...
        # ob['temporal_edges'] = np.tile(parent_ob['temporal_edges'], self.predict_steps+1)
    ...
    def render(self, mode='human'):
        ...
        for i in range(len(self.humans)):
            ...
            if -actual_arena_size <= self.humans[i].px <= actual_arena_size and -actual_arena_size <= self.humans[i].py <= actual_arena_size:
        ...
        corrs = []
        corrs.append(human_circles)
        corrs.append(robot)

        RG={}
        RG['RG_x'] = self.robot.gx
        RG['RG_y'] = self.robot.gy
        corrs.append(RG)
        # print("")
        return corrs
