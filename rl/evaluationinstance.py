...
def write_csv(filename, corrs):
    point_counts = len(corrs)

    points = np.zeros((22, 3, point_counts))
    ...
    for i in range(0, point_counts):
        points[20, 0, i] = corrs[i][1].center[0]
        points[20, 1, i] = corrs[i][1].center[1]
        points[20, 2, i] = corrs[i][1].radius
    for i in range(0, point_counts):
        points[21, 0, i] = corrs[i][2]['RG_x']
        points[21, 1, i] = corrs[i][2]['RG_y']
...
def evaluate(actor_critic, eval_envs, num_processes, device, test_size, logging, config, args, visualize=False):
    ...
    for k in range(test_size):
        ...
        formatted_k = str(k)
        write_csv("MatlabPlot/datainstance/"+formatted_k + ".csv", corrs_all)
