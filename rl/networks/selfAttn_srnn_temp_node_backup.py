...
class EdgeAttention_M(nn.Module):
    ...
    def create_h2r_attn_weight(self, robot_states, human_states, mask=None):
        """
        创建人对机器人的注意力矩阵，参数如下：
    ...
    def multi_policy_fusion(self, h2r_attn_weight, sxy_attn_weight):
        pass

    def weighted_attn_operation(self, h_spatials, attn_weight):
    ...
    def forward(self, h_temporal, robot_node, temporal_edges, h_spatials, spatial_edges, each_seq_len):
        ...
        Forward pass for the model
        params:
        h_temporal :    robot_states    :: Hidden state of the temporal edgeRNN -> robot states after linear layer
        ...
        for i in range(self.num_attention_head):
            ...
            h2r_attn_weight = self.create_h2r_attn_weight(temporal_embed, spatial_embed, mask=FOV_mask) # 仅仅考虑FOV内的人；可以替换为其他

            # Policy 2: 基于先验知识的 attn_weight
            attn_mask_sxy = self.create_priori_knowlege_attn_weight(human_points=spatial_edges, robot_points=robot_node, robot_speed=temporal_edges, mask=FOV_mask)

            # 多策略融合


            # Do weighted attention operation，采用attn_weight对人h_spatials做加权
            weighted_value, attn = self.weighted_attn_operation(h_spatials, h2r_attn_weight)
...
class selfAttn_merge_SRNN(nn.Module):
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        if self.args.sort_humans:
            ...
            hidden_attn_weighted, _ = self.attn(h_temporal=robot_states,
