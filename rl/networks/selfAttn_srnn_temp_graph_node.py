...
from .GNN_Models import GCN_Net, GAT_Net
import math
...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.heads = 8
        self.gat_conv = GAT_Net(num_node_features=self.in_channels, hidden_channels=self.hidden_channels, output_channels=self.output_channels , heads=self.heads)
    ...
    def construct_pyg_data(self, human_pred, mask, adj_weight=True):
        seq_len, nenv, max_human_num, _ = human_pred.size()
        nenv = seq_len * nenv
        human_pred_viewed = human_pred.view(nenv, max_human_num, -1)
        # graph data list: begin
        graph_data_list = []
        pygDataList = []
        # graph data list: begin

        for n_cpu in range(nenv):
            W_matrix = torch.zeros((max_human_num, max_human_num)).cuda()  # 权重矩阵

            human_current_cpu = human_pred_viewed[n_cpu, :, :]
            mask_current_cpu = mask[n_cpu, :]
            num_of_humans_masked = torch.sum(torch.eq(mask_current_cpu, True))

            for i in range(num_of_humans_masked):
                for j in range(num_of_humans_masked):
                    if i != j:
                        ps = human_current_cpu[i, :]
                        pd = human_current_cpu[j, :]
                        ...
                        v_vec[0] = vs2vd_x
                        v_vec[1] = vs2vd_y
                        v_vec_norm = torch.norm(v_vec)

                        cos_val = F.cosine_similarity(p_vec, v_vec, dim=0)
                        if cos_val >= 0:
                            weight_val = torch.zeros(1)
                        else:
                            if adj_weight:
                                weight_val = torch.neg(torch.mul(v_vec_norm, cos_val))
                            else:
                                weight_val = torch.ones(1)

                        W_matrix[j, i] = weight_val  # D to S

            nonzero_indices = torch.nonzero(W_matrix)
            nonzero_count = len(nonzero_indices)

            x = human_current_cpu
            edge_index = nonzero_indices

            # graph_data = {'node_features': x, 'edge_indices': edge_index}
            # graph_data_list.append(graph_data)

            data = Data(x=x, edge_index=edge_index.t().contiguous())
            pygDataList.append(data)


        # custom_dataset = CustomGraphDataset(graph_data_list)
        # print(" ")
        return pygDataList
    ...
    def forward(self, inp, each_seq_len):
        ...
        # add by zq: begin gat_conv
        nenv_pyg_data = self.construct_pyg_data(human_pred=inp, mask=attn_mask, adj_weight=True)
        z = self.gat_conv(nenv_pyg_data)
        z = torch.stack(z, dim=0)

        # print("nenv: ", nenv, 'nenv_pyg_data size :', len(nenv_pyg_data), "z.size : ", z.size())
        return z
        # add by zq: end  gat_conv

        # # original: begin
        # input_emb = self.embedding_layer(inp)
        # input_emb_viewed = input_emb.view(seq_len * nenv, max_human_num, -1)
        # input_emb_transposed = torch.transpose(input_emb_viewed, dim0=0, dim1=1)  # if we use pytorch builtin function, v1.7.0 has no batch first option
        # q = self.q_linear(input_emb_transposed)
        # k = self.k_linear(input_emb_transposed)
        # v = self.v_linear(input_emb_transposed)
        #
...
class selfAttn_merge_SRNN_Graph_Node(nn.Module):
    """
    ...
    def __init__(self, obs_space_dict, args, infer=False):
        ...
        infer : Training or test time (True at test time)
        """
        super(selfAttn_merge_SRNN_Graph_Node, self).__init__()
        self.infer = infer
        ...
        if self.args.use_self_attn:
            ...
            self.spatial_linear = nn.Sequential(init_(nn.Linear(512, 256)),
                                                nn.ReLU()
                                                )
        else:
            self.spatial_linear = nn.Sequential(init_(nn.Linear(obs_space_dict['spatial_edges'].shape[1], 128)),
                                                nn.ReLU(),
