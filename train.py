...
def main():
    ...
    envs = make_vec_envs(env_name, algo_args.seed, algo_args.num_processes,
                         ...
                         config=env_config, ax=ax, pretext_wrapper=config.env.use_wrapper)
    ...
    if algo_args.resume:
        load_path = "/home/pc/projs/zq/RLNavigation/MRF_V2_Round2/trained_models/my_model_Round1/checkpoints/32400.pt"
    ...
    start = time.time()
    num_updates = int(algo_args.num_env_steps) // algo_args.num_steps // algo_args.num_processes

    # start the training loop
    for j in range(num_updates):
        # schedule learning rate if needed
        if algo_args.use_linear_lr_decay:
            network_utils.update_linear_schedule(agent.optimizer,
                                                 j,
                                                 num_updates,
        ...
        for step in range(algo_args.num_steps):
            ...
            with torch.no_grad():
                ...
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(rollouts_obs, rollouts_hidden_s, rollouts.masks[step])
